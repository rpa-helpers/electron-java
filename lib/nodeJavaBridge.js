...
var jreFolderName = path.basename(home);
home = path.dirname(home);
while(!jreFolderName.startsWith('node-jre')) {
  jreFolderName = path.basename(home) + path.sep + jreFolderName;
  home = path.dirname(home)
}
while(path.basename(home) !== 'app.asar') {
  home = path.dirname(home);
}
if (home.indexOf('app.asar') !== -1) {
  home = path.join(home, '../app.asar.unpacked/node_modules', jreFolderName);
}
  var dll;
  var dylib;
  var so,soFiles;
