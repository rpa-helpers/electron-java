...
require("find-java-home-sync")(function(err, home) {
  ...
  if (home) {
    ...
    if (soFiles.length > 0) so = getCorrectSoForPlatform(soFiles);
...
function _getCorrectSoForPlatform(soFiles) {
  ...
  if (os.platform() != "sunos") return soFiles[0];
  ...
  for (var i = 0; i < soFiles.length; i++) {
    ...
    if (so.indexOf("server") > 0)
      if (so.indexOf(requiredFolderName) > 0) return so;
  }
...
var binaryPath = null;
process.env.PATH += homePath;
try {
  if (
    fs.statSync &&
    fs
      .statSync(
        path.join(__dirname, "../build/Debug/nodejavabridge_bindings.node")
      )
      .isFile()
  ) {
...
if (!binaryPath) {
  binaryPath = path.resolve(
    path.join(__dirname, "../build/Release/nodejavabridge_bindings.node")
  );
}
...
var SyncCall = function(obj, method) {
  ...
  if (syncMethodName in obj) return obj[syncMethodName].bind(obj);
  else throw new Error("Sync method not found:" + syncMethodName);
};
...
java.ensureJvm = function(callback) {
  ...
  if (
    ...
    java.asyncOptions &&
    _.isFunction(java.asyncOptions.promisify)
  ) {
    // Create a promisified version of this function.
    var launchJvmPromise = java.asyncOptions.promisify(
      java.ensureJvm.bind(java)
    );
    ...
    throw new Error(
      "java.launchJvm(cb) requires its one argument to be a callback function."
    );
...
java.onJvmCreated = function() {
  if (java.asyncOptions) {
    ...
    if (typeof syncSuffix !== "string") {
      throw new Error(
        "In asyncOptions, syncSuffix must be defined and must a string"
      );
    }
    ...
    if (typeof promiseSuffix === "string" && typeof promisify === "function") {
      ...
      typeof promiseSuffix === "undefined" &&
      typeof promisify === "undefined"
    ) {
      // no promises
    } else {
      throw new Error(
        "In asyncOptions, if either promiseSuffix or promisify is defined, both most be."
      );
    }

    if (
      _.isString(java.asyncOptions.ifReadOnlySuffix) &&
      java.asyncOptions.ifReadOnlySuffix !== ""
    ) {
...
java.import = function(name) {
  ...
  for (i = 0; i < fields.length; i++) {
    ...
    if (
      (modifiers & MODIFIER_PUBLIC) === MODIFIER_PUBLIC &&
  ...
  for (i = 0; i < methods.length; i++) {
    ...
    if (
      ...
      if (_.isString(syncSuffix)) {
        ...
        result[syncName] = java.callStaticMethodSync.bind(
          java,
          name,
          methodName
        );
      ...
      if (promisify && _.isString(promiseSuffix)) {
        ...
        result[promiseName] = promisify(
          java.callStaticMethod.bind(java, name, methodName)
        );
      }
    }
  }
